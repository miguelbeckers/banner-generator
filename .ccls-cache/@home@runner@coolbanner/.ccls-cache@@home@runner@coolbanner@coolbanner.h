...
void printBanner(char *text, struct TFont font, char *color, int tracking) {
  printf("%s", color);

  for (int i = 0; i < strlen(text); i++) {
    if (tracking > 0)
      moveCursorRight(tracking);
...
void printDefault(char text[]) {
  printBanner(text, BASIC_REGULAR, (char *)NO_COLOR, 1);
}

void printFont(struct TFont font) {
  printBanner((char *)"abcdefghijklm\n", font, (char *)NO_COLOR, 1);
  printBanner((char *)"nopqrstuvwxyz\n", font, (char *)NO_COLOR, 1);
  printBanner((char *)"ABCDEFGHIJKLM\n", font, (char *)NO_COLOR, 1);
  printBanner((char *)"NOPQRSTUVWXYZ\n", font, (char *)NO_COLOR, 1);
  printBanner((char *)"0123456789\n", font, (char *)NO_COLOR, 1);
}